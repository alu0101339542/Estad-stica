...
vector<int> HormigaInversa::Desplazamiento(Mundo &world) {
  Celda celda;
  vector<int> nextpos;
  int nextx = GetX();
   int nexty = GetY();
...
void HormigaInversa::Giro(Mundo &world) {
  Celda celda;
    celda = world.Getmatrixcel(GetX(), GetY());
...
void HormigaInversa::Movimiento(Mundo &world) { //Si vemos que la prox posicion es la ultima llamamos al metodo de la clase mundo con polimorfismo
  vector<int> next;
  int cambiodir;
  while(1) {
