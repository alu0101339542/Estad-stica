...
vector<int> HormigaC::Desplazamiento(Mundo &world) {
  Celda celda;
  vector<int> nextpos;
  int nextx = GetX();
   int nexty = GetY();
    celda = world.Getmatrixcel(GetX(), GetY());
      ...
      if(celda.Getcolor() == white) { //45º a la derecha
...
void HormigaC::Giro(Mundo &world) {
  Celda celda;
    celda = world.Getmatrixcel(GetX(), GetY());
...
void HormigaC::Movimiento(Mundo &world) { //Si vemos que la prox posicion es la ultima llamamos al metodo de la clase mundo con polimorfismo
  vector<int> next;
  int cambiodir;
  while(1) {
...
vector<int> HormigaD::Desplazamiento(Mundo &world) {
  Celda celda;
  vector<int> nextpos;
  int nextx = GetX();
   int nexty = GetY();
...
void HormigaD::Giro(Mundo &world) {
  Celda celda;
    celda = world.Getmatrixcel(GetX(), GetY());
...
void HormigaD::Movimiento(Mundo &world) { //Si vemos que la prox posicion es la ultima llamamos al metodo de la clase mundo con polimorfismo
  vector<int> next;
  int cambiodir;
  while(1) {
    world.PrintMundo(GetX(), GetY(), Getdir());
