...
class Vector{
  private:
    vector<Base_t> base_;                            // Array que contiene los elementos
    Index_t dimension_;                       // N�mero de elementos
    int min;
    int max;
  ...
  public:
    Vector(const Index_t size_left=0, const Index_t size_right=0);             // Constructor por defecto
...
//
template<class Base_t>
Vector<Base_t>::Vector(const Index_t size_left, const Index_t size_right): 
  dimension_(size_right - size_left), base_(0) {
  build();
}
...
template<class Base_t>
void Vector<Base_t>::Resizev(int new_size) {
